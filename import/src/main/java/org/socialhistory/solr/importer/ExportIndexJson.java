...
/**
 * Exports the Lucene document.resource fields as JSON.
 */
public class ExportIndexJson {
    ...
    public ExportIndexJson(String index) throws IOException, TransformerConfigurationException {
        final FSDirectory directory = FSDirectory.open(new File(index));
        reader = IndexReader.open(directory);


        TransformerFactory transformerFactory = TransformerFactory.newInstance();
        final InputStream resourceAsStream = this.getClass().getResourceAsStream("/marc2json.xsl");
    ...
    public static void main(String[] args) throws IOException, TransformerException {
        ExportIndexJson dirtyExporter = new ExportIndexJson(args[0]);
