<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="marc:datafield[@tag='245']/marc:subfield[@code='a' and text()='[Picture postcard.]']/text()">
        <xsl:variable name="nieuwe_titel">Photograph / </xsl:variable>
        <xsl:variable name="toevoeging_titel" select="normalize-space(../marc:datafield[@tag='500'][1]/marc:subfield[@code='a'])"/>
        <xsl:choose>
