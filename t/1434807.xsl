<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="marc:datafield[@tag='245' and marc:subfield[@code='a' and text()='[Jaarverslag = Annual report.]']]">

        <xsl:variable name="toevoeging_titel_a"
                      select="normalize-space(../marc:datafield[@tag='710'][1]/marc:subfield[@code='a'])"/>
        <xsl:variable name="toevoeging_titel_b"
                      select="normalize-space(../marc:datafield[@tag='710'][1]/marc:subfield[@code='b'])"/>

        <marc:datafield ind1="{@ind1}" ind2="{@ind2}" tag="245">
            <marc:subfield code="a">De nieuwe titel</marc:subfield>
