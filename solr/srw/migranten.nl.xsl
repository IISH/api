...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="migranten:migranten">
            <dl>
                ...
                <xsl:apply-templates select="migranten:soort"/>
                <xsl:apply-templates select="migranten:stichtingsjaar"/>
                <xsl:apply-templates select="migranten:doelstelling"/>
                <xsl:apply-templates select="migranten:plaats"/>
        </dl>
    </xsl:template>

    <xsl:template match="migranten:naam|migranten:kvknummer|migranten:soort|migranten:stichtingsjaar|migranten:doelstelling|migranten:plaats">
        ...
        <!-- Todo: introduct a label for the follow up query with a value that may be different from the search value.
        For example, the searchfield may be 1990(look for 1990) or unknown(look for 0) -->
        <xsl:variable name="labelvalue">
            <xsl:choose>
                <xsl:when test="local-name()='stichtingsjaar' and text()='-1'">Unknown</xsl:when>
            ...
            <xsl:call-template name="listitem">
                ...
                <xsl:with-param name="shortlist"><xsl:choose><xsl:when test="contains('naam soort doelstelling plaats', local-name())">yes</xsl:when><xsl:otherwise>no</xsl:otherwise></xsl:choose></xsl:with-param>
                <xsl:with-param name="explain" select="document('explain.migranten.nl.xml')"/>
