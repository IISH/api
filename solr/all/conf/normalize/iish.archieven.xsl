...
<xsl:stylesheet
            ...
            <extraRecordData>
                <iisg:iisg>
                    ...
                    <xsl:call-template name="beeld_en_geluid">
                        <xsl:with-param name="collection" select="ead:archdesc/ead:did/ead:repository/ead:corpname[normalize-space(text())='Nederlands Instituut voor Beeld en Geluid']"/>
                    </xsl:call-template>
                    ...
                    <marc:controlfield tag="008">
                        ...
                        <xsl:variable name="place">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="$tmp='GB'">xxk</xsl:when>
    ...
    <xsl:template match="@*|node()" mode="text">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="@*|node()" mode="text"/><xsl:text> </xsl:text>
                </xsl:copy>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
