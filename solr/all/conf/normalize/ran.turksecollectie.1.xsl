...
<xsl:stylesheet
    ...
    <xsl:template match="XMLDOCUMENT">
        ...
        <xsl:variable name="isShownBy"
                      select="concat('http://hosting25.deventit.nl/nijmegen/HttpHandler/icoon.ico?icoonfromxmlbeschr=',$identifier)"/>
        ...
        <record>
            <extraRecordData>
                <iisg:iisg>
                    ...
                    <iisg:isShownAt>
                        <xsl:value-of select="concat('http://hosting25.deventit.nl/nijmegen/AtlantisPubliek/migranten_detail.aspx?view=lijst&amp;xmldescid=', $identifier, '&amp;volgnummer=0&amp;globaal=&amp;mode=&amp;uitgebreid_zoeken=true&amp;positie=0')"/>
                    </iisg:isShownAt>
            ...
            <recordData>
                <marc:record xmlns:marc="http://www.loc.gov/MARC21/slim">
                    ...
                    <xsl:call-template name="insertElement">
                        ...
                        <xsl:with-param name="code" select="'a'"/>
                        <xsl:with-param name="value" select="BESCHRIJVINGEN/UITGEBREID"/>
                    </xsl:call-template>
                    <xsl:call-template name="insertElement">
                    ...
                    <xsl:call-template name="insertElement">
                        ...
                        <xsl:with-param name="code" select="'a'"/>
                        <xsl:with-param name="value" select="THEMA"/>
                    </xsl:call-template>
                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'650'"/>
                        <xsl:with-param name="code" select="'g'"/>
                        <xsl:with-param name="value" select="REGIO"/>
                    </xsl:call-template>
                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'260'"/>
                        <xsl:with-param name="code" select="'c'"/>
                        <xsl:with-param name="value" select="DATERING"/>
                    </xsl:call-template>
                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'650'"/>
                        <xsl:with-param name="code" select="'a'"/>
