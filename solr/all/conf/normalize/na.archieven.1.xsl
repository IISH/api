        ...
        <record>
            <extraRecordData>
                <iisg:iisg>
                    ...
                    <iisg:isShownAt>
                        <xsl:value-of
                                select="concat('http://www.nationaalarchief.nl/webviews/page.webview?eadid=NL-HaNA_', $identifier)"/>
                    </iisg:isShownAt>
                    <iisg:date_modified>
            ...
            <recordData>
                <marc:record xmlns:marc="http://www.loc.gov/MARC21/slim">
                    ...
                    <xsl:call-template name="insertElement">
                        ...
                        <xsl:with-param name="value" select="//archdesc/did/unitid"/>
                    </xsl:call-template>
                    ...
                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'245'"/>
                        <xsl:with-param name="code" select="'f'"/>
                        <xsl:with-param name="value" select="//archdesc/did/unitdate/name"/>
                    </xsl:call-template>

                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'110'"/>
                        <xsl:with-param name="code" select="'a'"/>
                        <xsl:with-param name="value" select="//archdesc/did/origination/corpname"/>
                    </xsl:call-template>

                    <xsl:call-template name="insertElement">
                        <xsl:with-param name="tag" select="'852'"/>
                        <xsl:with-param name="code" select="'a'"/>
                        <xsl:with-param name="value" select="//archdesc/did/repository"/>
                    </xsl:call-template>
                    ...
                    <xsl:call-template name="insertElement">
                        ...
                        <xsl:with-param name="code" select="'a'"/>
                        <xsl:with-param name="value" select="//archdesc/did/abstract"/>
                    </xsl:call-template>
