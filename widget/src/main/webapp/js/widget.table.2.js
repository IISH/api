...
function init() {
    ...
    // Add a search button:
    var search = $("<button type='submit' name='search' title='Search' value='Search' />");
...
function addSearchItem(displayLabel, index, relation, actualTerm, interfieldoperator) {
    ...
    if ($.trim(actualTerm).length != 0) {
        var searchComponent = {"displayLabel":displayLabel, "index":index, "relation":relation, "actualTerm":"\"" + encodeURIComponent(actualTerm) + "\"", "displayTerm":actualTerm, "interfieldoperator":interfieldoperator };
...
function carousel(dt, dd_images) {
    {
        $(dt).hide();
        var ol_carousel = $("<div class='jcarousel-skin-tango'/>");
        $.each(dd_images, function (ix, dd) {
            var img = $(dd).find("img");
            $(img.clone()).appendTo(ol_carousel);
            $(dd).remove();
        });
        var dd = $("<dd/>");
        $(dd).insertAfter(dt);
